...
if __name__ == "__main__":
    ...
    if (not os.path.exists("%s/pairwise_params" % dir)) or (not os.path.exists("%s/model_params" % dir)):
        ...
        # Get potential parameters. Don't include exluded volume terms.
        # Including excluded volume. Get some frames that blow up (?). Miscalculation of excluded volume term.
        #pairs = np.loadtxt("%s/pairwise_params" % dir,usecols=(0,1),skiprows=1,dtype=int)[2*n_native_pairs::] - 1
        #param_idx = np.loadtxt("%s/pairwise_params" % dir,usecols=(2,),skiprows=1,dtype=int)[2*n_native_pairs::]
        #pair_type = np.loadtxt("%s/pairwise_params" % dir,usecols=(3,),skiprows=1,dtype=int)[2*n_native_pairs::]
        if contacts == "native":
