...
for x in ${dirs}; do
    ...
    n_nat=$(expr $temp - 1)
    n_nonnat=`cat pairwise_params | sed "1d" | awk '{print $1, $2, 0.15}' | uniq | sed "${n_nat}d" | wc -l`
    echo "${n_nonnat}" > nonnative_contacts.dat
    cat pairwise_params | sed "1d" | awk '{print $1, $2, 0.5}' | uniq | sed "${n_nat}d" >> nonnative_contacts.dat
