...
def plot_QivsQ(name,iteration,Qbins,Qi_vs_Q,n_bins,epsilons,loops,state_bounds):
    ...
    for i in range(len(loops)):
        ...
        if (loops[index] > np.mean(loops)):
            ...
            ax = axes[1]
        #ax.plot(0.5*(Qbins[1:] + Qbins[:-1]),Qi_vs_Q[:,index],lw=2.5,alpha=0.5,color=cm.spectral((loops[index] - min(loops))/float(max(loops))))
    ...
    for i in range(len(loops)):
        ...
        #ax.plot(0.5*(Qbins[1:] + Qbins[:-1]),dQi_dQ[:,index],lw=2.5,alpha=0.5,color=cm.spectral((loops[index] - min(loops))/float(max(loops))))
...
def get_some_iteration_data(name,iteration,n_bins):
    ...
    Qbins, Qi_vs_Q,n_bins = get_contact_probability_versus_Q(name,iteration,n_bins)

    return epsilons, loops, n_residues, contacts, n_contacts, state_labels, state_bounds, Qbins, Qi_vs_Q, n_bins
...
if __name__ == "__main__":
    ...
    epsilons, loops, n_residues, contacts, n_contacts, state_labels, state_bounds, Qbins, Qi_vs_Q, n_bins = get_some_iteration_data(name,iteration,n_bins)
